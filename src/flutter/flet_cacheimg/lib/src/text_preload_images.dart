...
final imageExtensions = ['.png', '.jpg', '.jpeg', '.webp', '.bmp', '.gif'];

Future<void> precacheAllAssetsRecursively(BuildContext context, String assetsDirPath) async {
  if (_assetsPrecached) return;
  _assetsPrecached = true;

  final dir = io.Directory(assetsDirPath);

  if (!await dir.exists()) {
    print("Precached: ${dir} is not exists. Cant pre load images.");
    return;
  }


  await for (var entity in dir.list(recursive: true, followLinks: false)) {
    if (entity is io.File && imageExtensions.any((ext) => entity.path.toLowerCase().endsWith(ext))) {
      try {
        await precacheImage(FileImage(entity), context);
        print("Precached: ${entity.path}");
      } catch (e) {
        print("Failed to precache ${entity.path}: $e");
      }
    }
  }
}

class PreCacheTextControl extends StatelessWidget with FletStoreMixin {
  final Control? parent;
  final Control control;
  final bool parentDisabled;
  final FletControlBackend backend;

  const PreCacheTextControl({
    super.key,
  ...
  Widget build(BuildContext context) {
    return withPageArgs((context, pageArgs) {
      if (!_assetsPrecached) {
        precacheAllAssetsRecursively(context, pageArgs.assetsDir);
      }
